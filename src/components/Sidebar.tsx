    ...
    <aside className={`fixed left-0 top-16 bottom-0 w-64 border-r flex flex-col transition-colors duration-300 overflow-y-auto ${darkMode ? 'bg-gray-900 border-gray-800 text-white' : 'bg-white border-gray-100 text-gray-900'}`}>
      <div className="p-4 space-y-1 mt-2">
        {navItems.map(({ id, label, icon: Icon }) => (
          ...
          >
            ...
            <span>{label}</span>
            {currentView === id && <ChevronRight className="w-4 h-4 ml-auto opacity-60" />}
          </button>
        ))}
      </div>
      ...
      <div className={`mx-4 my-2 p-4 rounded-2xl border ${darkMode ? 'bg-gray-800/60 border-gray-700' : 'bg-gray-50 border-gray-100'}`}>
        <div className="flex items-center justify-between mb-3">
          <span className={`text-xs font-semibold uppercase tracking-wider ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>Today's Progress</span>
      ...
      <div className="mx-4 mt-2 grid grid-cols-2 gap-2">
        {[
          ...
          <div key={label} className={`p-3 rounded-xl border text-center ${darkMode ? 'bg-gray-800/60 border-gray-700' : 'bg-white border-gray-100'}`}>
